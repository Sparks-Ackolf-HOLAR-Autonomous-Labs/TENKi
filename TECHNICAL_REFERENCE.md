...
- **Layer C ??Theory and interpretation** (禮禮 10??4):
...
The **synthesis table** (禮 20) and **decision rule** (禮 21) are the primary outputs for
...
> **Important**: intersection ??symmetric. Study A (mixbox?命YB) has mean=(189,127,121) ??> far from the neutral gray (128,128,128). It is symmetric only under the narrow
...
Key fact from 禮17.1 of the parent TECHNICAL_REFERENCE: **62% of uniform RGB space falls outside
...
This is the most practically relevant criterion for the PEGK穩 framework because the
knowledge space type determines what transfer learning is possible (禮9??1 of parent doc).
...
| Coverage-uniform (庰-approximate) | **2??** | **5??0** | Volume-proportional sampling |
...
(禮10 of parent doc): when a policy trained on an asymmetric study D_i = G_A \ G_B transfers
...
phenomenon the PEGK穩 rating system was designed to detect.**
...
|-------|---------|---------|---------|----------|-----------|
| **A** (mixbox?命YB) | [22, 254] | [27, 242] | [30, 255] | (189, 127, 121) | Warm/pink ??artist consensus warm tones |
| **B** (spectral?YB) | [0, 255] | [134, 233] | [128, 221] | (162, 185, 175) | Cool/cyan-green ??physics-only colors |
| **C** (KM?ixbox) | [56, 209] | [62, 214] | [44, 207] | (164, 154, 81) | Earth tones ??oil-pigment-unique warm browns |
...
|-------|---------------|----------|----------------------|
| A (mixbox?命YB) | Intersection-asymmetric: skewed toward warm artist-consensus colors | (189,127,121) | 0.249 |
| B (spectral?YB) | Difference-asymmetric: skewed toward physics-exclusive cool colors | (162,185,175) | 0.105 |
| C (KM?ixbox) | Difference-asymmetric: skewed toward oil-pigment earth tones | (164,154,81) | 0.318 |
...
For practical purposes, 庰-approximate engine-permutation symmetry holds when:

```
? engine pair (i, j):  | Vol(G_i \ G_j) - Vol(G_j \ G_i) | / Vol(G_i ??G_j)  <  庰
```

If this holds for 庰 ??0.05 (5% imbalance), the studies are approximately symmetric.
...
From 禮17.7.4 of the parent TECHNICAL_REFERENCE: the parent PEGKi Nash-routing analysis concentrates
...
|---|------|------|---------------|------------|-------|-------------|
| 2 | 1 | 100 | 0.676 | **0.800** | ??.124 | 0.721 |
| 5 | 2 | 300 | 0.757 | **0.850** | ??.093 | 0.798 |
| 10 | 5 | 500 | 0.821 | **0.904** | ??.083 | 0.839 |
| 20 | 11 | 900 | 0.870 | **0.942** | ??.072 | 0.881 |
| 50 | 29 | 2100 | 0.917 | **0.974** | ??.057 | 0.923 |
| 100 | 58 | 4200 | 0.939 | **0.987** | ??.048 | 0.947 |
...
(bayesian_ei: 帢=0.017, bayesian_ucb: 帢=0.058, simulated_annealing: 帢=??.008).
...
The LF variance reduction that MFMC theory predicts only materialises when 帢 is large.
...
TrueSkill2 team model (禮13) is mathematically equivalent to **Bayesian model averaging under
...
| ryb      | +0.0533   | DONOR    |
| km       | ??.1072   | RECEIVER |
| study_a  | ??.2654   | RECEIVER |
| study_c  | ??.3093   | RECEIVER |

**At N=full** (all experiments), tau values converge to 0.61??.94: spectral and study_b remain the
best aligned; study_c and study_a have the lowest ceilings (0.61??.72).

**Key finding**: study_b (spectral?YB difference) is a nearly equal donor to spectral itself
at N=1. The intersection study (study_a, mixbox?命YB) is a strong receiver ??it provides little
...
**Cycle detection**: a directed edge A? is added when `external_gap(A,B) > 庰`.
...
| N | Ensemble tau | Swarm tau | Swarm advantage |
|---|---|---|---|
| 10  | 0.8417 | 0.8258 | **??.0158** |
| 20  | 0.8808 | 0.8614 | **??.0194** |
| 50  | 0.9197 | 0.8819 | **??.0378** |
| 100 | 0.9344 | 0.8881 | **??.0464** |
| 200 | 0.9461 | 0.8864 | **??.0597** |
| 500 | 0.9531 | 0.8853 | **??.0678** |
| 1000| 0.9494 | 0.8822 | **??.0672** |
...
acquisition decisions, closing the loop between the PEGKi transfer theory (禮10??1) and
...
|---|---|
| 禮10 Frugal twin convergence | `OfflineReplayObjective` replays the same tau-vs-N logic |
| 禮11 MFMC optimal acquisition | `MFMCPolicy` implements the r_l formula online |
| 禮12 Egalitarian vs heterogeneous | `EnsembleMFPolicy` (egalitarian) vs `SwarmMFPolicy` (heterogeneous) |
| 禮13 TrueSkill2 partial play | `BeliefState.effective_fidelity` / `quality_score` tracks per-source transfer usefulness |
| 禮15 Ensemble vs swarm | Formalized as distinct policy classes; run through same scheduler |
...
| study_b_rev ??spectral | 0.355 | ??.163 | 0.519 | ASYMMETRIC (largest) |
...
| mixbox | +0.1416 | DONOR |
| ryb | +0.0420 | DONOR |
| study_c_reverse | ??.0535 | RECEIVER |
| study_b_reverse | ??.0694 | RECEIVER |
| km | ??.1072 | RECEIVER |
| study_a | ??.1759 | RECEIVER |
| study_c | ??.2377 | RECEIVER |

**New finding**: the two reverse databases are mild receivers (??.05 to ??.07), weaker than km but
...
Jaccard overlaps between all study_* databases are near-zero (0.000??.027), confirming that
...
|---------|----------------|-------------|-------------|
| 10 | 0.764 | 0.578 | ??.186 quantity wins |
| 20 | 0.830 | 0.683 | ??.147 quantity wins |
| 30 | 0.856 | 0.746 | ??.110 quantity wins |
| 50 | 0.873 | 0.777 | ??.096 quantity wins |
| 100 | 0.900 | 0.850 | ??.050 quantity wins |
...
| 2 | 1 | 100 | 0.676 | **0.800** | ??.124 | 0.721 |
| 3 | 1 | 200 | 0.661 | **0.802** | ??.141 | 0.753 |
| 5 | 2 | 300 | 0.757 | **0.850** | ??.093 | 0.798 |
| 8 | 4 | 400 | 0.808 | **0.890** | ??.081 | 0.832 |
| 10 | 5 | 500 | 0.821 | **0.904** | ??.083 | 0.839 |
| 15 | 8 | 700 | 0.855 | **0.932** | ??.077 | 0.867 |
| 20 | 11 | 900 | 0.870 | **0.942** | ??.072 | 0.881 |
| 30 | 17 | 1300 | 0.891 | **0.959** | ??.067 | 0.912 |
| 50 | 29 | 2100 | 0.917 | **0.974** | ??.057 | 0.923 |
| 100 | 58 | 4200 | 0.939 | **0.987** | ??.048 | 0.947 |

**Verdict**: hybrid wins on tau at every budget. Classical also underperforms HF-only
on tau at every budget. This is the ranking-vs-estimation tradeoff (see 禮11):
...
| ryb | +0.1024 | DONOR |
| km | ??.0557 | RECEIVER |
| study_c_reverse | ??.1205 | RECEIVER |
| study_b_reverse | ??.1775 | RECEIVER |
| study_a | ??.2235 | RECEIVER |
| study_c | ??.2712 | RECEIVER |
...
See 禮15 for full table. **Ensemble beats swarm at every N** when all 8 non-spectral sources are
included. Swarm advantage degrades from ??.016 at N=10 to ??.068 at N=500.
...
- **Directional donor/receiver structure**: Experiment 02 produces the full asymmetric tau
  ...
  study_a and study_c are receivers (禮14, Experiment 02 results).
...
- **Async MF optimizer baseline**: Experiment 13 (ensemble_mf): 81 jobs, best_tau=0.769 at
  budget=50 with spectral HF + mixbox/km LF (禮16).
- **Parent PEGKi transfer analysis**: 210 transfer pairs computed; 171/210 robust (DRO 帤=0.5);
  Conjecture 2.1 PARTIALLY_SUPPORTED, Conjecture 2.2 NOT_SUPPORTED (禮17).
- **TENKi-1000 series**: All experiments 02??3 re-run with 1000-target databases and
  N=[10,20,50,100,200,500,1000] (禮18).
...
- **Fixed-budget diversity (Q4 partial)**: Experiment 07 with truly fixed N_TOTAL shows
  tau monotonically *decreases* as k increases (0.794 ??0.610 for k=1??).  With
...
- **Ensemble vs swarm at K=8 (Q7 partial)**: Experiment 12 shows ensemble beats swarm
  ...
  (std=0.242??.354 for set-op sources) does not translate to local quality ??swarm
...
2. **Approximate symmetry threshold**: At what 庰 does the 3-study KS-balanced collection
...
# See 禮18 for full commands. Quick single run:
...
| SymScore | Symmetry score of a study collection (禮6) |
| KS | Knowledge Space |
